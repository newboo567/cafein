	...
	<div class="container">
            ...
            if ($result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) { 
        	        ...
        	        echo "
                    <p id=\"example\" class=\"lead card-text text-shadow text-white w-50 d-none d-lg-block\">주문시간: ".$time_h."시 ".$time_i." 분</br>주문 상태: ". $out."</p>";
        	        if($row["isOut"]==0)
        	        {
        	            echo "<a href=\"orderComplete.php?idx=".$row["idx"]."\" class=\"btn btn-secondary\">제조완료</a>";
        	            echo "\t<a href=\"payComplete.php?idx=".$row["idx"]."\" class=\"btn btn-secondary\">결제완료</a>";
        	            
        	        }
        	        else if($row["isOut"]==1)
        	        {
        	            echo "<a href=\"#\" class=\"btn btn-secondary disabled\">제조완료</a>\t";
        	            echo "<a href=\"payComplete.php?idx=".$row["idx"]."\" class=\"btn btn-secondary\">결제완료</a>";
                    }
        	        echo "</div></div></div>";
        	    }
...
-->
    ...
    <footer class="bg-faded text-center py-5">
      <div class="container">
        ...
        <a href="logout.php" class="btn btn-secondary" >관리자 로그아웃</a>
 ...
 <script language='javascript'> 
  ...
  window.setTimeout('window.location.reload()',3000); //5초마다 리플리쉬 시킨다 1000이 1초가 된다. 
