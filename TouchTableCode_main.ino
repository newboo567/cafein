...
char tempFile[13];  //갑작스런 주문 안내를 위해 백업
boolean wasCanSkip; //갑작스런 주문 안내를 위해 백업
////////////////////////////섹터 판별 관련 전처리기 지정//////////////////////////////
...
void MusicMode();
void OrderMode();
void ChangeDir(char *, int, boolean);
void RSPGame();
void printPath();
void FindSector();
...
void setup() {
  ...
  InitPath();  //path 초기화
  OpenFileAndPlaySound("welcome.mp3", false);
}
/////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////전체 프로그램 동작///////////////////////////////////
...
void ActMainFunc() {
  ...
  if (powerOn) { //전원이 켜져있는 경우
    ...
    if (ISMANUALBUT) { //메뉴얼 관련 섹터(다시듣기, 홈버튼, 취소버튼, 전원버튼)
      ...
      currentMode = sector / 5; //music = 2, game = 3, order = 4
      PlayGuideTrack(SELECT); //선택되었다는 음성 출력
     //모드에 따라 기능을 수행
      switch (currentMode) {
        case MUSICMODE : //음악모드
          while (currentMode == MUSICMODE) {
            MusicMode();
          }
          break;
        case GAMEMODE : //게임모드
          while (currentMode == GAMEMODE) {
            GameMode();
          }
          break;
        case ORDERMODE : //주문모드
